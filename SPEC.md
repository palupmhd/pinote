...
**Status audit (per gelombang, bukan sapuan merata):** Sesi audit ini menyisir 34 area berdasar risiko — keamanan → integritas data/sync → aksesibilitas → responsif mobile → performa — bukan seluruh permukaan fitur secara setara. 5 bug ditemukan & diperbaiki (2 keamanan di atas; konflik sync palsu dari perubahan non-dokumen seperti seleksi/pan; baca cloud tanpa filter `user_id`; fokus keyboard hilang total di 24 field karena `outline-none` tanpa pengganti; kontrol TopBar/Toolbar terpotong di layar <640px). Performa diuji langsung (PerformanceObserver, 0 long-task di 120 kartu selagi pan/zoom/drag) — sehat, tak ada temuan. **Area yang SENGAJA belum diaudit gelombang ini** (bukan berarti aman, cuma belum disentuh): Connector (ghost cleanup/kaskade hapus), detail editing Note/Task-list, Export PNG, offline queue, Presentation Mode UI (cuma logic urutan yang diuji), Minimap, Template starter, DnD Kanban/Kalender (dibangun & diuji sesi lain, tak di-re-audit), kaskade hapus Relation/panah, cleanup "buka baris sebagai kanvas", Backlinks/mention, migrasi IndexedDB, dan sync multi-device sungguhan (butuh kredensial nyata). Kalau lanjut audit, ini daftar antreannya.
