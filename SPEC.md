...
**Area yang SENGAJA masih belum diaudit** (bukan berarti aman, cuma belum disentuh): detail editing Note/Task-list, offline queue, Presentation Mode UI lanjutan (selain smoke test di atas), Minimap, Template starter (isi templat, bukan cuma tombolnya), DnD Kanban/Kalender (dibangun & diuji sesi lain, tak di-re-audit), Backlinks/mention, migrasi IndexedDB, dan sync multi-device sungguhan (butuh kredensial nyata). Kalau lanjut audit, ini daftar antreannya.
