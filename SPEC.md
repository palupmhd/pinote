...
**Area yang SENGAJA masih belum diaudit** (bukan berarti aman, cuma belum disentuh): Presentation Mode UI lanjutan (selain smoke test urutan), Minimap, Template starter (isi templat, bukan cuma tombolnya), DnD Kanban/Kalender (dibangun & diuji sesi lain, tak di-re-audit), migrasi IndexedDB, dan sync multi-device sungguhan (butuh kredensial nyata / login magic-link sungguhan yang tak bisa diotomasi di sini). Kalau lanjut audit, ini daftar antreannya.
