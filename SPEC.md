...
- **Grid & snap-to-grid diperbaiki, batas kanvas kiri/atas (gaya Milanote):** warna dot grid diganti dari tan/beige (`#d8d4c8`, kurang bersih menurut pemilik) ke abu netral (`#c9c9c8`). Jarak grid sempat dicoba 24px, tapi pemilik kasih referensi visual grid yang lebih rapat — final-nya **20px** (`GRID` di `lib/types.ts`), dipilih bukan cuma dari selera tapi karena hampir semua lebar kartu bawaan (`lib/store.ts`: `BOARD_CARD_WIDTH` 200, `TASK_LIST_WIDTH` 260, `LINK_WIDTH` 240, `DATABASE_CARD_WIDTH` 220) sudah kelipatan bersihnya; `NOTE_WIDTH` disesuaikan dari 248→240 biar ikut kelipatan juga. Efeknya: kartu attach ke dot bukan cuma di pojok kiri-atas, tapi tepi KANAN-nya pun jatuh pas di kolom dot yang sama (diverifikasi lewat Playwright: `left`/`width` hasil drag selalu kelipatan 20). Dot sebelumnya tidak align dengan titik snap kartu — `radial-gradient` defaultnya center-of-tile, jadi dot visual ada di kelipatan `GRID+GRID/2`, sementara snap membulatkan ke kelipatan `GRID` bersih — diperbaiki dengan menggeser `background-position` setengah ubin (`applyCamera` di `Canvas.tsx`) supaya keduanya align. **Kiri/atas kanvas kini tepi keras** (x=0/y=0 tak pernah kelihatan di luar itu — `clampCamera` di `lib/geometry.ts`, dipakai baik di ref kamera live (`Canvas.tsx`) maupun `setCamera` store): pan tak bisa melewati tepi itu. Kalau drag kartu (`moveMany`) mendorong kartu ke koordinat negatif, bukan ditolak/dipotong — SEMUA kartu papan itu digeser bareng sejumlah overflow-nya (kamera TIDAK dikompensasi, supaya pergeserannya kelihatan), efeknya kanvas terasa "melebar" ke arah itu. Kanan/bawah tetap tak terbatas. Diverifikasi manual via Playwright (drag lintas-tepi → kartu mendarat pas di batas & tetap terlihat penuh; pan lanjut ke arah tepi → diam di tempat). **Belum dikerjakan:** kartu baru lewat `addNote`/dbl-klik dekat tepi bisa saja langsung lahir sedikit negatif (belum lewat `moveMany`, jadi belum ikut ter-normalisasi) — belum jadi masalah nyata karena kecil & jarang, dicatat sebagai follow-up kalau kelak jadi isu.
