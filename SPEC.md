...
- **Model panah konektor disempurnakan** (keluhan pemilik: kepala panah "aneh") — `curveBetween` (`lib/geometry.ts`) sebelumnya SELALU melengkung horizontal-dulu apa pun arah relatif kedua kartu; untuk pasangan kartu yang hubungannya lebih vertikal (atas-bawah), ini memaksa kurva S menyamping, bikin kepala panah nempel di ujung yang condong ke samping alih-alih tegak lurus ke kartu tujuan. Diperbaiki: arah lengkung sekarang ikut sumbu yang jaraknya lebih dominan (horizontal ATAU vertikal, dipilih dari `|dx|` vs `|dy|`) — konsisten dengan `edgePoint` yang juga sudah memilih tepi keluar radial berdasar arah yang sama, jadi keduanya kini selalu sepakat. Lantai `pull` (jarak lengkung minimum) diturunkan dari 28 ke 16px supaya kartu yang berdekatan tak dapat lengkungan besar yang tak perlu. Bentuk kepala panah diganti dari segitiga rata jadi dart bertakik (belakang sedikit cekung) yang lebih tirus, ukurannya dinaikkan sedikit (markerWidth/Height 6→7) dan `refX` disesuaikan (9→8.5) biar tak overshoot berlebihan dari ujung kurva. Diverifikasi via Playwright: konektor antara dua kartu yang disusun vertikal sekarang menghasilkan path dengan kontrol-titik yang bergeser di sumbu Y (bukan X), kepala panah mengarah tegak lurus ke bawah alih-alih menyamping.
