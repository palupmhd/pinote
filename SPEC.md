...
- **Belum & butuh keputusan desain:** Structured⇄Spatial dual-mode (§7.2) — bergantung apakah row jadi Nested Board (desain spec asli) atau tetap `{id,cells}` (implementasi sekarang). Rollup/Formula preset, Backlinks (§10.3), plus polish drag-and-drop untuk Kanban/Kalender. **Tanya pemilik sebelum tiap item.**
